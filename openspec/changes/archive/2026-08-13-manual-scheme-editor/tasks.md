...
- [x] 3.5 实现自动保存弱提示 UI：触发保存时页面底部呈现浅灰色「已自动保存」非模态提示，并定时消失
- [x] 3.6 实现拦截退出与弹窗确认：通过 `page-container` 或自定义导航栏监听返回事件，拦截并弹窗确认「是否保留本次编辑？」
...
- [x] 5.2 方案表格 UI 与卡片操作重构：折叠持有期为右上角下拉菜单（取代「二手·n年」）；右下角增加「复制」与「垃圾桶（删除）」并排图标（图标文件已准备在 `miniapp/wx/assets/icons`），点击删除需二次确认并从数组彻底移除
- [x] 5.3 买入价输入校验与边界防御：0 < 数字 <= 999,999，越界或非法时高亮提示「买入价不合法」并标记该行不参与重算
- [x] 5.4 方案数量上限防爆栈拦截：检查并限制当前方案总数 <= 500，超限时阻断复制与新增操作并 toast 提示
- [x] 5.5 卡片复制与唯一 ID 渲染：点击复制深拷贝当前方案属性，并生成全新的唯一 ID（UUID或时间戳）确保 WXML 列表渲染 key 唯一
