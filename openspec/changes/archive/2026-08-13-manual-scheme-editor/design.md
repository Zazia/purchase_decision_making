...
**自动保存 UI 与退出拦截**：自动保存触发时，通过页面底部的自定义非模态 view 呈现浅灰色「已自动保存」提示并定时消失，不阻断操作。退出拦截通过 `page-container` 或是自定义导航栏的返回事件监听实现，当有未提交修改时弹窗提示「是否保留本次编辑？」。

**为何快照式而非命令式（记录每个操作）**：编辑动作类型多（改价/渠道/持有期/复制/删除/条件筛选/新增等），命令式 undo 需要为每类动作写逆操作，易漏且难测。快照式用空间换正确性，单快照 < 50KB（与 `saved-results` 单条预估一致），20 步撤销 < 1MB，在 `wx.setStorageSync` 单 key 1MB 限制内。若担心超限，可限制 history 上限为 30 步、超出时丢弃最旧。
...
5. **持有期下拉折叠**：将原来的「二手·n年」右上角标签替换为「持有期」Picker 下拉菜单。用户可直接修改持有期属性，而无需将不同持有期展平为多张卡片，极大减少了卡片数量（从 $N \times 4$ 变为 $N \times 1$）。
6. **复制与删除**：在卡片右下角提供并排的「复制」和「删除（垃圾桶）」图标。相关图标已放置于 `miniapp/wx/assets/icons` 目录下（`copy.svg` 和 `delete (2).svg`）。复制时对当前 `EditedPlanPoint` 进行深拷贝，并生成一个全局唯一的 `id`，插入到当前卡片下方；删除时要求二次确认，确认后彻底移出状态数组（不是设置 `deferred=true`，而是从 DOM 和数组中直接抹除）。
7. **「暂不考虑」→「移到末尾」**：列表底部的批量操作或滑动操作（如有）文案从「暂不考虑」改为「移到末尾」，语义和技术实现（`deferred=true`）不变。
...
**决策**：
1. **防爆栈与 OOM 限制（最大数量 500）**：在执行复制和新增时，检查 `points.length >= 500`，超限则阻断交互并提示。500 是考虑到 `wx.setStorageSync` 1MB 限制和 canvas 绘制长图的高度限制得出的安全天花板。
2. **大数极值防御**：买入价输入绑定最大值 999,999。防止由于输入 10 亿等极端值导致的科学计数法解析错误或布局破版。
3. **WXML 列表渲染唯一键**：原先引擎产出的结果数组可能用自增 index 即可。引入复制操作后，由于复制方案的绝大部分属性一致，必须引入一种唯一标识生成机制（如时间戳+随机数），确保 `wx:key` 绑定独一无二，防止视图组件复用时状态窜乱。
