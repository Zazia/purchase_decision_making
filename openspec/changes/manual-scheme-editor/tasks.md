...
- [ ] 4.5 用户修改版保存：在结果页点击「保存结果」时，调用 `onGenerateShareCard`，将 `userModified` 写入 `globalData.shareCardData`，跳转至 share-card 页面进行分享和保存长图，并生成独立 id 快照，不覆盖原快照

## 5. 小程序：方案编辑器交互（改价/渠道/筛选/排除/移到末尾/新增）

- [ ] 5.1 编辑器表头 UI：左侧「← 返回」图标，右侧「↩ 撤销」图标，中间保留「重算报告」与「导出长图」。去掉标题文字和「上传分享」按钮
- [ ] 5.2 方案表格 UI：去掉买入价/渠道文字标签，买入价 input 前加「￥」符号。行尾操作改为「排除」复选框和「移到末尾」按钮
- [ ] 5.3 买入价输入校验：> 0 数字，非法时高亮提示「买入价需大于 0」并标记该行不参与重算
- [ ] 5.4 渠道 picker 与默认值：新品默认「京东」（带「国补」复选框），二手默认「闲鱼」。渠道列表含「淘宝/拼多多/爱回收」等。选「其他」时显示下划线文本输入框供自行填写
- [ ] 5.5 条件筛选面板：在列表上方实现多维度复选框组（芯片/内存/存储/持有期/系列/前沿/价格区间）。系列通过 `scheme-editor-state` 从 model 或 categoryKey 解析。取消勾选项自动将其对应方案标记 `deferred=true` 移到末尾
- [ ] 5.6 移到末尾（原暂不考虑）分组：移入后折叠到分组视图，提供「恢复」按钮回到主列表原位置（并同步恢复相关筛选勾选）
- [ ] 5.7 新增自定义方案表单：机型名/芯片/内存/存储/买入时机/买入价/持有期；标注「自添加」
- [ ] 5.8 所有编辑动作（改价/渠道/排除/恢复/移到末尾/筛选/新增/删除）统一走 `scheme-editor-state.push`，确保撤销栈覆盖

## 6. 小程序：重算帕累托图与报告切换

- [ ] 6.1 「重算报告」按钮：收集未排除、未被筛选移出的 `EditedPlanPoint[]`，调 `engine-bridge.recomputeFromEditedPlans`
- [ ] 6.2 重算结果写入 `data.userModified`，切换 `viewMode='userModified'`，顶部显示「基于用户输入价」
- [ ] 6.3 全部方案被排除或筛除时提示「当前没有可重算的方案，请恢复或新增至少一个方案」，不进入空报告
...
- [ ] 10.1 上传分享入口调整：将原编辑器表头的「上传分享」按钮与重算后的提示弹窗，移至分享卡页面的分享流程中或在重算/导出长图后触发
