...
引擎单测 fixtures SHALL 反映 skill 历史产出的 HTML 报告场景。相同输入下，引擎计算结果 MUST 与生成该报告时所用模型版本的 skill 报告帕累托点一致（月均成本误差 ≤ 0.5 元，性能满足度误差 ≤ 0.001）。模型口径升级（如 v4.3 残值买入价锚定）后，历史报告数值 MUST NOT 追改，一致性校验按各自时点的模型版本口径进行。

#### Scenario: 与历史报告一致

- **WHEN** 用 skill 历史 v3.7 报告的输入参数调用引擎（该报告生成于 v3.7 模型口径时点）
- **THEN** 引擎按 v3.7 口径的回归 fixtures 返回的前沿点坐标与历史报告中帕累托图的点位一致，误差在阈值内
