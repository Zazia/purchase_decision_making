 ...
 */
export function computeWaitMonths(
  releasePlan: ReleasePlan,
  macroContext?: MacroContext,
): number {
  if (!releasePlan.nextReleaseMonth) return 0;
  const analysisMonth = resolveAnalysisMonth(macroContext, undefined);
  if (!analysisMonth) return 0;
 ...
 */
export function predictNewProductPrice(
  constants: Constants,
  categoryKey: string,
  releasePlan: ReleasePlan,
): number {
 ...
 */
export function predictDiscountedOldPrice(
  constants: Constants,
  oldCandBuyPrice: number,
  releasePlan: ReleasePlan,
  macroContext?: MacroContext,
): number {
  const historicalMean = lookupImpactMean(constants, releasePlan.category);
  if (historicalMean <= 0) return oldCandBuyPrice;
 ...
 */
export function resolveProductReleaseDate(
  constants: Constants,
  releaseDateKey: string,
): string | undefined {
  let releaseDate: string | undefined = constants.productReleaseDates[releaseDateKey];
 ...
 */
export function isAnchorCandidate(
  constants: Constants,
  releaseDateKey: string,
  releasePlan: ReleasePlan,
): boolean {
 ...
 */
export function computeAdjustedImpact(
  constants: Constants,
  releasePlan: ReleasePlan,
  macroContext?: MacroContext,
): number {
 ...
 */
export function computeResidualImpactFactor(
  constants: Constants,
  releaseDateKey: string,
  releasePlan: ReleasePlan,
  ...
  const next = releasePlan.nextReleaseMonth;
  if (!next) return 1;
  if (isAnchorCandidate(constants, releaseDateKey, releasePlan)) return 1;
  const analysisMonth = resolveAnalysisMonth(macroContext, constants.lastUpdated);
  if (!analysisMonth) return 1;

  const monthsToRelease = diffMonths(analysisMonth, next);
  if (monthsToRelease < 0 || monthsToRelease > holdingMonths) return 1;

  const adjustedImpact = computeAdjustedImpact(constants, releasePlan, macroContext);
  if (adjustedImpact <= 0) return 1;
 ...
 */
export function lookupImpactTimeVaryingFactor(
  constants: Constants,
  monthsSinceRelease: number,
): { residualFactor: number; buyPriceDropFactor: number } {
  ...
  if (entry && typeof entry === 'object') {
    const e = entry as { 残值调整因子?: number; 买入价下降因子?: number };
    return {
 ...
 */
export function shouldGenerateWaitCandidates(
  releasePlan: ReleasePlan,
  macroContext?: MacroContext,
): boolean {
  if (!releasePlan.nextReleaseMonth) return false;
  if (releasePlan.releaseConfidence === 'low') return false;
  const analysisMonth = resolveAnalysisMonth(macroContext, undefined);
  if (!analysisMonth) return false;
  const monthsToRelease = diffMonths(analysisMonth, releasePlan.nextReleaseMonth);
...
function parseDays(s: unknown): number {
 ...
 *   无效 → null
 */
function parseReleaseMonth(s: unknown): string | null {
  ...
  if (quarterMatch) {
    ...
    const qMap: Record<string, number> = { '一': 1, '二': 4, '三': 7, '四': 10 };
...
function diffMonths(fromMonth: string, toMonth: string): number {
...
/** 按月数映射到冲击时变曲线的时段键 */
function resolvePeriod(months: number): string {
  if (months <= 1) return '1月内';
  if (months <= 3) return '3月内';
  if (months <= 6) return '6月内';
  if (months <= 12) return '12月内';
  return '12月后';
}

const TIME_VARYING_FALLBACK: Record<string, { residualFactor: number; buyPriceDropFactor: number }> = {
...
/** 查找品类的发布时间预测置信度 */
function lookupConfidence(constants: Constants, categoryKey: string): 'high' | 'medium' | 'low' {
  const validation = constants.releaseTimeValidation;
  if (!validation) return 'low';
  // 键名带日期后缀 (如 _当前校验结果_2026-08-02), 取第一个以 _当前校验结果_ 开头的子表
  let results: Record<string, { 置信度?: string }> | undefined;
  for (const [k, v] of Object.entries(validation)) {
    if (k.startsWith('_当前校验结果_') && v && typeof v === 'object') {
      results = v as Record<string, { 置信度?: string }>;
      break;
    }
  }
...
function lookupDelay(
  constants: Constants,
  categoryKey: string,
): { baselineDelayDays: number; pessimisticDelayDays: number } {
  const table = constants.waitPeriodModel?._分品类上市到货延迟_基线;
  const entry = lookupByCategory(table, categoryKey);
  if (!entry) return { baselineDelayDays: 0, pessimisticDelayDays: 0 };
  return {
 ...
 */
function lookupPriceHike(
  constants: Constants,
  categoryKey: string,
): { median: number; hasHikeOccurred: boolean; isAnnounced: boolean } {
  const table = constants.pricePredictionModel?._分品类预测涨幅表;
  if (!table) return { median: 0, hasHikeOccurred: false, isAnnounced: false };
  // 键名带日期后缀 (如 _当前值_2026-08), 取第一个以 _当前值_ 开头的子表
  let current: Record<string, { 预测涨幅?: string; 中位数?: string }> | undefined;
  for (const [k, v] of Object.entries(table)) {
    if (k.startsWith('_当前值_') && v && typeof v === 'object') {
      current = v as Record<string, { 预测涨幅?: string; 中位数?: string }>;
      break;
    }
  }
  if (!current) return { median: 0, hasHikeOccurred: false, isAnnounced: false };
  const entry = lookupByCategory(current, categoryKey);
  if (!entry) return { median: 0, hasHikeOccurred: false, isAnnounced: false };
...
function lookupImpactMean(constants: Constants, categoryKey: string): number {
  const table = constants.newReleaseImpact as Record<string, unknown> | undefined;
  const entry = lookupByCategory(table, categoryKey);
  if (!entry || typeof entry !== 'object') return 0;
  const e = entry as { 均值?: string };
  return parsePercent(e.均值);
}

/** 按新品涨幅查价格传导因子 (返回小数, 如 -0.35) */
function lookupTransmissionFactor(constants: Constants, hike: number): number {
  ...
  const raw = table[key];
  if (typeof raw === 'string') return parsePercent(raw);
  // 表缺失时回退默认
  return defaultTransmissionFactor(hike);
}

function resolveTransmissionKey(hike: number): string {
  const pct = hike * 100;
  if (pct <= 0) return '新品涨幅≤0%';
  if (pct <= 5) return '新品涨幅0-5%';
  if (pct <= 15) return '新品涨幅5-15%';
  if (pct <= 30) return '新品涨幅15-30%';
  return '新品涨幅>30%';
}

function defaultTransmissionFactor(hike: number): number {
  const pct = hike * 100;
  if (pct <= 0) return 0;
...
function isMacroTriggered(macroContext?: MacroContext): boolean {
...
function lookupMacroCapacityFactor(constants: Constants, macroContext?: MacroContext): number {
...
const STAGE_TO_CHINESE_KEY: Record<MacroContext['storageSuperCycleStage'], string> = {
  ongoing: '存储超级周期_进行中',
  peaking: '存储超级周期_见顶中',
  easing: '存储超级周期_缓解',
  none: '无宏观事件',
...
const DEFAULT_MACRO_CAPACITY_FACTOR: Record<MacroContext['storageSuperCycleStage'], number> = {
  ongoing: 2.0,
  peaking: 1.5,
  easing: 1.0,
  none: 1.0,
 ...
 * - 未触发宏观事件用 0
 */
function resolveHikeForTransmission(
  releasePlan: ReleasePlan,
  macroContext?: MacroContext,
): number {
  if (!isMacroTriggered(macroContext)) return 0;
  return releasePlan.predictedPriceHike ?? 0;
}

/** 解析分析月: 优先 macroContext.analysisMonth, 否则用 constants.lastUpdated */
function resolveAnalysisMonth(
  macroContext: MacroContext | undefined,
  constantsLastUpdated: string | undefined,
): string | null {
  if (macroContext?.analysisMonth) return macroContext.analysisMonth;
