...
// ============================================================================
// Constants 类型 (对应 constants.json v3.8 结构)
// ============================================================================
...
export interface ChipBenchmarks {
  Mac芯片?: Record<string, { 单核: number; 多核: number; GPU_OpenCL?: number; [k: string]: unknown }>;
  iPhone_iPad芯片?: Record<string, { 单核: number; 多核: number; [k: string]: unknown }>;
}

/** 芯片代际提升假设 */
export interface ChipGenerationAssumptions {
  M系列?: { r: number; [k: string]: unknown };
  A系列?: { r: number; [k: string]: unknown };
}
...
/** 月均成本计算公式(参考用) */
export type CostFormula = { [k: string]: unknown };
...
/** 设计 token */
export type DesignTokens = { [k: string]: unknown };
